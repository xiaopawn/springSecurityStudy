...
public interface MyUserDetailsServiceMapper {
    ...
    @Select("select username,password,enable from sys_user where username = #{username}")
    MyUserDetails findByUserName(@Param("username") String username);

    @Select("select role_code from sys_role r left join sys_user_role sur on r.id = sur.role_id left join sys_user su on sur.user_id = su.id where su.username = #{username}")
